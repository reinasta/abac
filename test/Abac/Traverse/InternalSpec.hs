...
spec = describe "Traverse.Internal module" $ do
  ...
  it "Get the determiners from a paragraph's AST" $ do
    determiners1 `shouldBe` determiners_right

  it "Get the adjectives from a paragraph's AST" $ do
    adjectives1 `shouldBe` adjectives_right

  it "Get the adverbs from a paragraph's AST" $ do
    adverbs1 `shouldBe` adverbs_right

  it "Get the adjectives and the adverbs from a paragraph's AST (using string endings)" $ do
    adwords_tagged1 `shouldBe` adwords_right

  it "Get the past participles from a paragraph's AST" $ do
    pastParticiples1 `shouldBe` pastParticiples_right
...
adwords_right :: [Inline]
adwords_right = [Word [Tag "JJ",None] (2,81) "abominable",Word [Tag "JJ",None] (4,9) "unacceptable",Word [Tag "JJ",None] (4,30) "egregious"]

--tests: looking for inline having various tags
determiners1 :: [Inline]
determiners1 = determinersInBlock pnlptag1_result :: [Inline]

determiners_right :: [Inline]
determiners_right = [Word [Tag "DT",None] (1,3) "the",Word [Tag "DT",None] (1,21) "this",Word [Tag "DT",None] (1,44) "all",Word [Tag "DT",None] (2,4) "The",Word [Tag "DT",None] (2,41) "the",Word [Tag "DT",None] (4,0) "That"]

pastParticiples1 :: [Inline]
pastParticiples1 = pastParticiplesInBlock pnlptag1_result :: [Inline]

pastParticiples_right :: [Inline]
pastParticiples_right = [Word [Tag "VBN",None] (1,69) "horrified",Word [Tag "VBN",None] (2,24) "been",Word [Tag "VBN",None] (2,29) "returned",Word [Tag "VBN",None] (2,106) "used",Word [Tag "VBN",None] (4,54) "horrified",Word [Tag "VBN",None] (5,5) "flabbergasted"]

adjectives1 :: [Inline]
adjectives1 = adjectivesInBlock pnlptag1_result :: [Inline]

adjectives_right :: [Inline]
adjectives_right = [Word [Tag "JJ",None] (2,81) "abominable",Word [Tag "JJ",None] (4,9) "unacceptable",Word [Tag "JJ",None] (4,30) "egregious"]

adverbs1 :: [Inline]
adverbs1 = adverbsInBlock pnlptag1_result :: [Inline]

adverbs_right :: [Inline]
adverbs_right = [Word [Tag "RB",None] (2,100) "badly",Word [Tag "RB",None] (4,47) "really"]


-- toc gives the embarassing 'empty list' error on docs/examples/alice.md
